...
func TestDirKeyStorage_Put(t *testing.T) {
	...
	certBytes, _ := ioutil.ReadFile(filepath.Join(getTestDir(), "dir_keystorage", "issued/good_cert.crt"))
	if !bytes.Equal(certBytes, []byte("certbytes")) {
		t.Errorf("DirKeyStorage.Put() wrong cert bytes in result file")
	}
	certBytes, _ = ioutil.ReadFile(filepath.Join(getTestDir(), "dir_keystorage", "certs_by_serial/9A.crt"))
	if !bytes.Equal(certBytes, []byte("certbytes")) {
		t.Errorf("DirKeyStorage.Put() wrong cert bytes in result file")
	}
	certBytes, _ = ioutil.ReadFile(filepath.Join(getTestDir(), "dir_keystorage", "ca.crt"))
	if !bytes.Equal(certBytes, []byte("certbytes")) {
		t.Errorf("DirKeyStorage.Put() wrong cert bytes in result file")
	}
	keyBytes, _ := ioutil.ReadFile(filepath.Join(getTestDir(), "dir_keystorage", "private/good_cert.key"))
