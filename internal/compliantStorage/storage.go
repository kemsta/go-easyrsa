...
// DirKeyStorage is an easyrsa v3 compliant storage. It can be used as a drop replacement on the PKI created with easyrsa v3
...
func (s *DirKeyStorage) Put(pair *pair.X509Pair) error {
	...
	if err != nil {
		if !os.IsNotExist(err) {
			return fmt.Errorf("%s already exists. Cancel writing to avoid overwriting this file", certPath)
		}
	}
...
func (s *DirKeyStorage) GetByCN(cn string) ([]*pair.X509Pair, error) {
	...
	certBytes, err := os.ReadFile(filepath.Join(s.pkidir, "issued", fmt.Sprintf("%s.crt", cn)))
	if err != nil {
		return nil, fmt.Errorf("can't read cert by cn %s: %w", cn, err)
	}
	keyBytes, err := os.ReadFile(filepath.Join(s.pkidir, "private", fmt.Sprintf("%s.key", cn)))
	if err != nil {
